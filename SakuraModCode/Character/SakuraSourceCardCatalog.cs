...
internal sealed record SakuraSourceCardMetadata(
    Type CardType,
...
internal static class SakuraSourceCardCatalog
{
    private static readonly IReadOnlyList<SakuraSourceCardMetadata> EntriesInternal = BuildEntries();
    private static readonly IReadOnlyDictionary<Type, SakuraSourceCardMetadata> EntriesByType =
    ...
    public static IReadOnlyList<SakuraSourceCardMetadata> Entries => EntriesInternal;
    ...
    public static bool TryGetMetadata(CardModel card, out SakuraSourceCardMetadata metadata) =>
        TryGetMetadata(card.GetType(), out metadata);

    public static bool TryGetMetadata(Type type, out SakuraSourceCardMetadata metadata) =>
        EntriesByType.TryGetValue(type, out metadata!);

    public static SakuraSourceCardMetadata MetadataFor(Type type) =>
    ...
    internal static void ValidateEntries(IReadOnlyList<SakuraSourceCardMetadata> entries)
    {
    ...
    private static IReadOnlyList<SakuraSourceCardMetadata> BuildEntries()
    {
        var entries = new List<SakuraSourceCardMetadata>();
