...
public sealed class IllusionMonster : WindMonsterTemplate
{
    ...
    internal void ReshufflePresentation()
    {
        var images = CombatState.Enemies
        ...
        var other = Rng.NextItem(images.Where(creature => creature != Creature).ToList());
        if (other is not null)
            Visuals.IllusionVisualController.ExchangePositions(Creature, other);
        Visuals.IllusionVisualController.SetRealBodyRevealed(Creature, revealed: false);
    }
    ...
    {
        await DamageCmd.Attack(BeguilingDamage).FromMonster(this).WithNoAttackerAnim().Execute(null);
    ...
    private Task LuredFall(IReadOnlyList<Creature> targets) =>
        DamageCmd.Attack(LuredFallDamage).FromMonster(this).WithNoAttackerAnim().Execute(null);
