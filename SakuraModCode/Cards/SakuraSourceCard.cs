...
public abstract class SakuraSourceCard(
    int cost,
    CardType type,
    CardRarity rarity,
    TargetType target) :
    ModCardTemplate(cost, type, rarity, target)
{
    ...
    internal bool IsSpellCard => this is SpellCard;
    internal bool ShowsEnergyCost => this is SpellSeal or SpellRelease || !IsSpellCard;

    public virtual SakuraElementSet Elements => SakuraElementSet.None;
...
public abstract class ClowCard(
    int cost,
    CardType type,
    CardRarity rarity,
    TargetType target) :
    SakuraSourceCard(cost, type, rarity, target);

public abstract class ClowExtraEffectCard :
    ClowCard,
    ISakuraExtraEffectCard
{
    protected ClowExtraEffectCard(
        int cost,
        CardType type,
        CardRarity rarity,
        TargetType target) :
...
public abstract class SakuraFormCard(
    int cost,
    CardType type,
    TargetType target) :
    SakuraSourceCard(cost, type, CardRarity.Token, target)
{
    public override int MaxUpgradeLevel => 0;
    public override bool CanBeGeneratedInCombat => false;
}

public abstract class SakuraEventCard(int cost, CardType type, TargetType target) :
    SakuraSourceCard(cost, type, CardRarity.Event, target)
{
    public override int MaxUpgradeLevel => 0;
}

public abstract class SakuraAncientCard(
    int cost,
    CardType type,
    TargetType target) :
    SakuraSourceCard(cost, type, CardRarity.Ancient, target)
{
...
public abstract class SpellCard(int cost, CardType type, CardRarity rarity, TargetType target) :
    SakuraSourceCard(cost, type, rarity, target)
{
    public override bool CanBeGeneratedInCombat => false;
...
internal static class SakuraSealKillPolicy
{
    public static bool IsSealCard(CardModel? card) =>
        card is SpellSeal or GrowingMagic;
}

internal static class SakuraSourceCardRules
{
    ...
    public static int ConvertedSakuraCount(Player owner) =>
        owner.Deck.Cards.OfType<SakuraFormCard>().Select(card => card.Identity).Distinct().Count()
    ...
    public static int StarterClowCount(Player owner, SourceCardIdentity identity) =>
        Math.Clamp(owner.Deck.Cards.OfType<ClowCard>().Count(card => card.Identity == identity), 1, 4);
...
internal static class SakuraStarterScaling
{
    ...
    {
        var count = SakuraSourceCardRules.StarterClowCount(owner, identity);
        var rate = Math.Min(MaxLoneRate, Math.Max(0m, (BaseStarterCount - count) * LoneRate));
        return (int)Math.Floor(baseValue * (1m + rate));
    }
}

internal interface ISakuraSourceEffectiveValueVar
{
    int EffectiveValue(CardModel card);
}

internal static class SakuraSourceCardValues
{
    public static int EffectiveValue(CardModel card, DynamicVar variable) =>
        variable is ISakuraSourceEffectiveValueVar effectiveValueVar
            ? effectiveValueVar.EffectiveValue(card)
            : variable.IntValue;
}

internal sealed class SakuraSourceDamageVar : DamageVar, ISakuraSourceEffectiveValueVar
{
    private readonly SourceCardIdentity? _starterIdentity;

    public SakuraSourceDamageVar(decimal damage, ValueProp props, SourceCardIdentity? starterIdentity = null) :
        base(damage, props)
    {
        _starterIdentity = starterIdentity;
    }

    public SakuraSourceDamageVar(string name, decimal damage, ValueProp props, SourceCardIdentity? starterIdentity = null) :
    ...
    public int EffectiveValue(CardModel card)
    {
        ...
        {
            return SakuraReleaseState.AdjustedReleasedValue(
                card,
                Name,
                baseValue,
                value => SakuraStarterScaling.ScaledValue(card.Owner, identity, value));
        }
...
internal sealed class SakuraCombatHistoryDamageVar(
    decimal damage,
    ValueProp props,
    Func<CardModel, int> hitCount,
    string perHitVarName = SakuraSnowRules.PerCardDamageVar) : DamageVar(damage, props)
{
    public override void UpdateCardPreview(CardModel card, CardPreviewMode previewMode, Creature? target, bool runGlobalHooks)
    {
...
internal sealed class SakuraCombatHistoryCountVar(Func<CardModel, int> hitCount) : DynamicVar("Magic", 0)
{
    public override void UpdateCardPreview(CardModel card, CardPreviewMode previewMode, Creature? target, bool runGlobalHooks) =>
        PreviewValue = Math.Max(0, hitCount(card));
}

internal sealed class SakuraSourceBlockVar(decimal block, ValueProp props, SourceCardIdentity? starterIdentity = null) :
    BlockVar(block, props), ISakuraSourceEffectiveValueVar
{
    public override void UpdateCardPreview(CardModel card, CardPreviewMode previewMode, Creature? target, bool runGlobalHooks)
    {
        decimal baseValue = EffectiveValue(card);
...
internal static class SakuraCreateRewards
{
    ...
    {
        var relic = SakuraRelicCatalog.TryPullRandomAvailableForCreate(owner);
        if (relic is null)
        {
...
internal static class SakuraReleaseState
{
...
internal static class SakuraSourceAssetPaths
{
...
internal static class SakuraCombatHistory
{
...
internal static class SakuraSnowRules
{
    ...
    internal static bool CountsAsWateryCard(CardModel card) =>
        SakuraActions.HasElement(card, SakuraElement.Water);
    ...
    private static int PlayedCards(CardModel card, Func<CardModel, bool> predicate) =>
        card.Owner is null || card.CombatState is null
            ? 0
            : SakuraCombatHistory.PlayedCardsThisCombat(card.Owner, predicate);
}
