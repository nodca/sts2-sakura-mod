...
internal static class ClassicCardVisualAssets
{
    ...
    public static IEnumerable<string> RunAssetPaths(SakuraSourceCard card)
    {
    ...
    public static string FullFacePath(SakuraSourceCard card) =>
        FullFacePath(card, ArtStem(card.GetType()).NormalClassicArtStem());

    public static string UnknownFullFacePath(SakuraSourceCard card) =>
        FullFacePath(card, "unknown.png");

    public static string FullFacePath(SakuraSourceCard card, string fileName) =>
    ...
    public static string? EnergyIconPath(SakuraSourceCard card) =>
        card.IsClowCard
            ? ClassicCardEnergyIcon.ClowBigPath
            : card.IsSakuraCard
                ? ClassicCardEnergyIcon.SakuraBigPath
                : card is SpellSeal or SpellRelease or SpellTurn
                    ? ClassicCardEnergyIcon.ClowBigPath
                    : null;

    public static string? TextEnergyIconPath(SakuraSourceCard card) =>
        card.IsClowCard
    ...
    internal static string FullFaceFamilyDirectory(SakuraSourceCard card) =>
        card.IsClowCard
            ? "clow"
            : card.IsSakuraCard
                ? "sakura"
    ...
    private static bool IsFullFaceCard(SakuraSourceCard card) =>
...
internal static class ClassicCardLayout
{
    ...
    {
        if (card.Model is not SakuraSourceCard model
            || !IsClassicCard(card)
        ...
        var facePath = card.Visibility == ModelVisibility.Visible
            ? ClassicCardVisualAssets.FullFacePath(model)
            : ClassicCardVisualAssets.UnknownFullFacePath(model);
        var faceTexture = ClassicCardVisualAssets.Texture(facePath);
    ...
    private static void ApplyCardLayout(NCard card, ClassicCardState state)
    {
        ...
        if (model.ShowsEnergyCost)
        {
            ApplyCostLayout(
                ...
                ClassicCardVisualAssets.EnergyIconPath(model));
        }
        else
        {
        ...
        bool visible = true)
    {
        ...
        ApplyBox(layer, box);
        if (visible)
            SetTextureIfDifferent(layer, ClassicCardVisualAssets.Texture(path));
        ...
        SakuraCardMutationLedger ledger)
    {
        ...
        if (flash is TextureRect textureRect)
            SetTextureIfDifferent(textureRect, ClassicCardVisualAssets.Texture(ClassicCardVisualAssets.FlashPath()));
    ...
    private static void ApplyEnglishNameLayout(Label label, SakuraSourceCard model, bool visible)
    {
        ...
        string? iconPath = null)
    {
        ...
        if (icon is not null)
        {
            ...
            if (iconPath is not null)
                SetTextureIfDifferent(icon, ClassicCardVisualAssets.Texture(iconPath));
        }
    ...
    private static Rect2 EnglishNameBox(SakuraSourceCard model) =>
        model.IsSakuraCard
    ...
    {
        var artStem = ClassicCardVisualAssets.ArtStem(cardType).NormalClassicArtStem();
        ...
        SubViewport? TransformVfxViewport)
    {
        public static ClassicLayoutContext For(NCard card)
        {
            ...
            for (var parent = card.GetParent(); parent is not null; parent = parent.GetParent())
            {
                ...
                if (parent is NCardTransformVfx or SpellTurnCardPreviewContainer)
                {
                    context = priority < 3
    ...
    private sealed class ClassicCardLayoutSpec
    {
        ...
        public int TitleFontSize => SakuraCardVisualStyle.NativeTitleFontSize;
        public int TitleMinFontSize => SakuraCardVisualStyle.NativeTitleMinFontSize;
        public int TitleMaxFontSize => SakuraCardVisualStyle.NativeTitleMaxFontSize;
        public int NameTextOutlineSize => SakuraCardVisualStyle.NativeTitleOutlineSize;
        public int NameTextShadowOffset => SakuraCardVisualStyle.NativeTitleShadowOffset;
        public int NameTextShadowOutlineSize => SakuraCardVisualStyle.NativeTitleShadowOutlineSize;
