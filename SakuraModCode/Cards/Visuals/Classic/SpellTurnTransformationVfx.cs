...
internal sealed partial class SpellTurnCardPreviewContainer : Control;

internal static class SpellTurnTransformationTimeline
{
    ...
    [
        SpellTurnTransformationVfx.AudioPath("SOTE_SFX_Buff_1_v1.ogg"),
        SpellTurnTransformationVfx.AudioPath("SOTE_SFX_Buff_2_v1.ogg"),
        SpellTurnTransformationVfx.AudioPath("SOTE_SFX_Buff_3_v1.ogg")
    ];
}

internal static class SpellTurnTransformationVfx
{
    internal const string ScenePath = MainFile.ResPath + "/scenes/combat/spell_turn_transformation_vfx.tscn";
    internal const string LuminPath = MainFile.ResPath + "/images/vfx/spell_turn_lumin.png";
    internal const string SfxRoot = MainFile.ResPath + "/sfx/spell_turn";
    ...
    internal sealed class Session : IDisposable
    {
        ...
        public async Task<bool> PlayPrelude()
        {
            ...
            enlargeTween.TweenProperty(_center, "scale", Vector2.One, SpellTurnTransformationTimeline.EnlargeEnd)
                .SetEase(Tween.EaseType.Out)
                .SetTrans(Tween.TransitionType.Cubic);
            if (!await WaitActive(SpellTurnTransformationTimeline.EnlargeEnd))
                return false;

            PlayAudio(
                _primaryAudio,
                TurnAudioPath,
                SpellTurnTransformationTimeline.LoweredPitchScale);
            PlayAudio(
                _accentAudio,
            ...
            return await WaitActive(
                SpellTurnTransformationTimeline.SwitchStart
                - SpellTurnTransformationTimeline.EnlargeEnd);
        }
        ...
        public async Task PlayReveal(CardModel endCard)
        {
            ...
            switchTween.TweenMethod(
                    ...
                    0f,
                    1f,
                    SpellTurnTransformationTimeline.SwitchDuration)
                .SetEase(Tween.EaseType.InOut)
                .SetTrans(Tween.TransitionType.Sine);
            if (!await WaitActive(SpellTurnTransformationTimeline.SwitchDuration))
                return;
            ...
            PlayAudio(
                ...
                SpellTurnTransformationTimeline.CompletionAudioPaths[
                    _random.RandiRange(0, SpellTurnTransformationTimeline.CompletionAudioPaths.Length - 1)],
                1f);
            SpawnDiffusionTail();

            if (!await WaitActive(
                    SpellTurnTransformationTimeline.ShrinkStart
                    - SpellTurnTransformationTimeline.RevealStart))
            {
                return;
            }
            ...
            shrinkTween.TweenProperty(_center, "scale", Vector2.One * 0.666f, 0.25f)
                ...
                .SetTrans(Tween.TransitionType.Cubic);
            await WaitActive(SpellTurnTransformationTimeline.TotalDuration
                             - SpellTurnTransformationTimeline.ShrinkStart);
        }

        private static NCard CreatePreview(Control container, CardModel card)
        {
            var origin = new SpellTurnCardPreviewContainer
            {
            ...
            var preview = NCard.Create(card)
                ?? throw new InvalidOperationException($"Could not create Spell Turn preview for {card.Id}.");
        ...
        private async Task AnimateGatherParticles()
        {
            ...
            var elapsed = 0f;
            while (elapsed < SpellTurnTransformationTimeline.GatherDuration && IsActive())
            {
                var progress = Mathf.Clamp(
                    elapsed / SpellTurnTransformationTimeline.GatherDuration,
                    0f,
                    1f);
        ...
        private async Task AnimateDiffusionTail(Node2D tail)
        {
            ...
            var elapsed = 0f;
            while (elapsed < SpellTurnTransformationTimeline.DiffusionDuration
                   && GodotObject.IsInstanceValid(tail)
                   && tail.IsInsideTree())
            {
                var progress = Mathf.Clamp(
                    elapsed / SpellTurnTransformationTimeline.DiffusionDuration,
                    0f,
                    1f);
                for (var index = 0; index < directions.Length; index++)
                {
