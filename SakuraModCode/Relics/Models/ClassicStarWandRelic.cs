...
public class ClassicStarWandRelic : ClassicSealedWandRelic
{
    ...
    public override bool IsAllowed(IRunState runState) =>
        runState.Players.Any(static player => SakuraUltimateWandRecipe.FindExactSealedWand(player) is not null);

    public override async Task AfterObtained()
    {
        var sealedWand = SakuraUltimateWandRecipe.FindExactSealedWand(Owner);
        if (sealedWand is not null && !ReferenceEquals(sealedWand, this))
        {
            var replacement = SakuraUltimateWandRecipe.CreateStarWandReplacement(sealedWand);
